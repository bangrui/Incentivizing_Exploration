...
$\sqrt{0.6 J \cdot \log (\log_{1.1}(J) + 1) + \frac{J x^2}{16 \sigma^2}}
\leq \sqrt{0.6 J \cdot \log (\log_{1.1}(J) + 1)} + \frac{J x}{4 \sigma}$ (here we use the fact $\sqrt{J}\leq J$).

Then, we use the following technical Lemma~\ref{lem:n0-inequality}
(whose proof is given in Appendix~\ref{lem:n0-inequality-proof}, and whose
...
&\ARMNUM \EvenLaterPhase
  + 24 \ARMNUM^2 d \cdot \frac{1}{1 - \exp \left(
    \frac{-1.8 s \cdot \MinProb^2}{256 \TieDensity^2 \Diam^2 d^2 \sigma^2}
  \right)} \nonumber \\
=& O\left( \max\{\frac{\ARMNUM \TieDensity^3 \Diam^3 d^3 \sigma^3}{\MinProb^3}, N\exp(\frac{2}{p})\}
...
\begin{align}
&E\left[\sum_{s=1}^{T}\sum_{t=t_{s-1}+1}^{t_{s}} \TieDensity \cdot g^2(s)\right] \leq \sum_{s=1}^{T}\TieDensity\cdot g^2(s)N\log(s). \label{equ:small_regret_bound}
\end{align}
...
\leq &\sum_{i,j}\Prob{|S_s|\geq 2\sqrt{\frac{2}{1.8}s\log(T)}} \\
\leq &\sum_{i,j}\Prob{|S_s|\geq \sqrt{0.6s\log(\log_{1.1}(s)+1)+\frac{2}{1.8}\log(T)s}} \leq 24\ARMNUM d \cdot \exp(-2\log(T)),
\end{align*}
where the second to last inequality is because the following technical inequality, whose proof we include in Appendix~\ref{lem:n1-inequality-proof}.
...
\lefteqn{\sum_{s=1}^{T} 24\ARMNUM^2 d \log(s) \MAXR \exp\left(-2\log(T)\right)  +
          \frac{128\ARMNUM\Diam^2 d^2\sigma^2
          \TieDensity\log(T)}{1.8}\sum_{s=1}^{T}\frac{\log(s)}{s}}
\\ & \leq
24\ARMNUM^2 d R\frac{T(T-1)}{2T^2}  + 71.11 \ARMNUM \Diam^2 d^2\sigma^2
          \TieDensity(\log(T))^2(\log(T)+1)
  \\ & \leq
12 \ARMNUM^2 d R  + 71.11 \ARMNUM\Diam^2 d^2\sigma^2 \TieDensity(\log(T))^2(\log(T)+1). 
\end{align*}

Adding all terms gives an upper bound of $O\left(\MAXR\cdot N\exp\left(\frac{2}{p}\right) \right) +O(\ARMNUM\Diam^2 d^2\sigma^2 \TieDensity(\log(T))^3)$ on the cumulative expected regret.
