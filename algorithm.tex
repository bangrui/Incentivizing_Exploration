...
\begin{itemize}
\item For those user who will pull an arm that occurs regret greater than $g(t)$: if all arms estimation errors are within $\frac{g(t)}{2\Diam d}$, then based on the same argument as \eqref{equ:choice}, this will never happen. We define a sequence of stopping time $\tau_{s}^{k}$ to be the \Kth{k}
...
\item For those user who will pull an arm that occurs regret smaller than $g(t)$: if the user pulls his/her best arm, no regret occurs. Otherwise, we know his/her utility difference between best and second best arm is smaller than $g(t)$. Further, we know this happens with probability \AlmostTied{g(t)} at each time,
