...
\begin{align}
&E[\sum_{s=1}^{T}\sum_{t=t_{s-1}+1}^{t_{s}} \TieDensity \cdot g^2(s)] \leq \sum_{s=1}^{T}\TieDensity\cdot g^2(s)Ns. \nonumber 
\end{align}
...
+ & \sum_{s = 1}^T 
  \Prob{\exists i,j, s.t. |\ArmE{t}{i}{j} - \Arm{i}{j}|\geq \frac{g(s)}{2\Diam d}} \cdot \MAXR
  \cdot \ARMNUM s,
+ \sum_{s=1}^{T}\TieDensity \cdot g^2(s)\cdot Ns.
\end{align*}
...
\lefteqn{\sum_{s=1}^{T} 24\ARMNUM^2 d s \MAXR \exp\left(-2\log(T)\right)  +
          ...
          \TieDensity\log(T)}{1.8}\sum_{s=1}^{T}\frac{1}{s}}
\\ & \leq
