...
Our main result is that Algorithm~\ref{alg:basic-incentivizing} achieves $O \left(\MAXR\cdot  N\exp(\frac{2}{p})\right) +
O(\ARMNUM\Diam^2 d^2\sigma^2 \TieDensity(\log(T))^3)$ cumulative regret with $O \left(\ARMNUM^2 \MAXR \Diam^2 \TieDensity d^3 \sigma\cdot \exp(\frac{2}{\MinProb}) \right)$ payment budget.  This is stated in the following pair of theorems.
