...
\right).}\nonumber 
\end{align}
\pfcomment{Maybe we want to simplify the constants in the above? Not sure.}
\end{rtheorem}
...
= 
& O \left(\ARMNUM^2  d \cdot (\MAXR + \Diam d \sigma)\cdot \max\bigg\{\exp\left(\frac{2}{p}\right),
		    \pfedit{\left( \frac{\sigma \Diam d}{\TieCutoff} \right)^{5/2},}
    \left( \frac{\sigma \TieDensity \Diam d}{\MinProb} \right)^{5/2}\bigg\}
\right).
\end{align*}
}
