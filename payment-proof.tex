...
\bcedit{
\begin{align*}
O \left(\ARMNUM\cdot (\MAXR + \Diam d \sigma) \cdot h(1) \right)
& = 
    O \left(\ARMNUM (\MAXR + \Diam d \sigma)\cdot \max\bigg\{\exp(\frac{2}{p}),
    \left( \frac{48 \sigma \ell \TieDensity \Diam d}{\MinProb} \right)^{5/2}\bigg\}
\right).
\end{align*}
}
\end{extraproof}
