...
\dkdelete{discrete} agent distributions \dkedit{with finite support}.
\dkdelete{In fact, in the case of a discrete agent distribution,}
\dkedit{Alternatively, when $\TieDensity = 0$, 
one can modify} the algorithm to
reduce the dependence on \MinProb from exponential to polynomial,
\dkedit{at the cost of reintroducing the poly-logarithmic dependence
  on $T$}.
...
\dkcomment{Adjust these bounds.}
\dkedit{Whether one can achieve regret that is simultaneously
  independent of $T$ and polynomial in \ARMNUM and \MinProb is an
