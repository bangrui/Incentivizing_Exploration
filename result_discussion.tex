...
constant in $T$;
thus, the algorithm achieves constant regret using constant payment. 
...
The instance is \pfdelete{very} simple: it has two arms, one \pfedit{with}
...
the modified algorithm \pfedit{has} expected regret at most
...
(\pfedit{This} setting violates our Assumption~\ref{A3},
...
surpasses \pfedit{the $\Theta(\log(T))$ achievable in the} standard multi-armed bandit.
This may seem surprising, because the
\pfedit{principal in our setting} has both less control and less information than in the
standard bandit setting.
The result arises because heterogeneity in preferences \pfdelete{indeed} provides
free exploration, \pfedit{and allows} all of the arms \pfedit{to} be pulled infinitely
often without incurring regret \pfedit{once estimates are accurate enough}.
...
Indeed, \pfedit{suppose} agents are still heterogeneous,
but the principal \pfedit{pulls} arms directly.
Unless \pfedit{the principal can also observe}
the agents' types, \pfedit{she} will be unable to correctly choose
each agent's preferred arm,
even with infinite exploration of arm attributes.
\pfedit{Regret will then} 
grow as $\Omega(T)$. 
