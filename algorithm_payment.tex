...
\pfedit{and is equal to $\hat{x}$ defined in Lemma~\ref{lem:no-incentives}}).
\pfedit{We set} the cutoff point between early and late phases to
...
\pfdelete{
Fix an arm $i$, and consider the \Kth{s} pull of arm $i$,
...
\pfedit{Fix an arm $i$, let $s \geq \EvenLaterPhase$, and 
