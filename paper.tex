...
We can also apply our models to scientific organizations like eBird \cite{frazier2014incentivizing}. EBird is a website which guides the enthusiasts to explore and report their findings to the bird community. In each report, it contains information such as when, where and how they go birding as well as what kinds of birds they see and hear. As the orginization, EBird needs to incentivize the enthusiasts to explore the less-explored parts of domain. For each enthusiast, he/she may have different preference towards the location's sight-seeing, weather, distance and safety etc. By offering enthusiasts incentivies, eBirds can obtain more reports about the less-explored area and have more information for the bird community.\bccomment{How about this eBird application?}
...
In this problem context, we study a simple policy that usually exploits, incentivizing agents to pull an arm only when the set of agent utility functions that would pull this arm without incentives has probability below a time-varying threshold. In our paper, we assume all arms are some agents' best arm. Under this assumption, we prove with $O(N^2)$ payment budget, we can bound the cumulative expected regret by $O(N^2+M(\log(T))^2)$ where $M$ is the upper bound of the marginal density for those agents who are indifferent between their best and the second best arm. In the case that $M=0$, which typically happens when the user preference distribution is discrete, we can achieve constant cumulative expected regret $O(N^2)$. The key difference between our problem setting and the homogenous preference setting (or the traditional multi-armed bandits setting) is that we do not need to incentivize the agents to try a suboptimal arm very often since all arms are some agents's best arm. These results suggest that heterogeneous agent preferences reduces but does not eliminate the need to incentive exploration, in relation to single-preference settings. Essentially, heterogeneity provides a certain amount of exploration for free. \bccomment{How about now?}
