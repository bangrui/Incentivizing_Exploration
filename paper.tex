...
= &  \sum_{i}\sum_{j} P(|\epsilon_{i,\tau}^{j}|>N(i,\tau)\lambda|\theta_{\tau}\cdot(u_{B(\theta_{\tau})}-u_{\hat{B}(\theta_{\tau})})\geq 2Wm\lambda). \label{ACI}
\end{align}

\pfcomment{where is $\epsilon^j_{i,\tau}$ defined?  has it been defined yet?}
