...
We structure our paper as follows: Section~\ref{sec:prob} formulates our problem; Section~\ref{sec:ub} states our algorithm and proves that we can achieve $O(N^2+(\log(T))^2)$ regret with $O(N^3)$ incentive budget \pfcomment{This should be $N^2$, right?  Also, do you want to add $M$ in here?}; Section~\ref{sec:lb} constructs an example showing regret is $\Omega(\log(T))$ in the worst case, regardless of incentive budget.
...
\FOR{ $t = 1, 2, 3, \cdots$}{
  ...
  \IF {$S$ is non-empty}
    \STATE{Choose an arm $i$ uniformly at random from $S$}
  ...
  \ENDIF
  \STATE Update $u_{i,t}$ and $N(i,t)$ for the pulled arm
...
In this secion, we discuss several assumptions that we need to acheive our results.  Denote
