...
We now bound $P(\omega\in L[l]|t\in[t_{n},t_{n+1}))$.
\begin{align}
&P(\omega\in L[l]|t\in[t_{n},t_{n+1})) \nonumber \\
=&P(\omega\in L^{'}[l]|t\in[t_{n},t_{n+1}))- P(\omega\in L^{'}[l-1]|t\in[t_{n},t_{n+1})) \nonumber \\
\leq & 1-P(|\epsilon_{i,t}^{j}|<\frac{l-1}{n}, \forall i,j|t\in[t_n,t_{n+1})) \nonumber \\
=&P(\exists i,j, s.t. |\epsilon_{i,t}^{j}|\geq \frac{l-1}{n}|t\in[t_n,t_{n+1})) \nonumber  \\
=&\sum_{i,j}P(|\epsilon_{i,t}^{j}|\geq \frac{l-1}{n}|t\in[t_n,t_{n+1})) \nonumber \\
=&\sum_{i,j}P(|S_{i,t}^{j}|\geq \frac{N(i,t)(l-1)}{n}|t\in[t_n,t_{n+1})) \nonumber 
\end{align}
...
\begin{proof}
If $|u_{i}^{j}-u_{i,t}^{j}|< \lambda$ is true $\forall i$, $\forall j$, then we know for those $\theta\in \{\theta:\theta\cdot u_{B(\theta)}-\max_{j\neq B(\theta)}\{\theta \cdot u_{j}\}\geq 2mW\lambda\}$, they will correctly identify their best arm. Thus we know, in the $n^{th}$ round, if $|u_{i}^{j}-u_{i,t}^{j}|< \frac{p^{-1}(\frac{p}{2})}{2Wm}$ $\forall i$ and $\forall j$, and $n^{-1}\leq p/2$, we do not need to incentivize any arms. In order to have $n^{-1}\leq \frac{p}{2}$, we need $n\geq \frac{2}{p}$. Denote $n_1=\max\{n_{0}, \frac{2}{p}\}$. Denote $\delta_{0}=p^{-1}(\frac{p}{2})>0$ (because of Assumption~\ref{A1}).The cumulative expected number of payments is bounded above by:

Define $\tau_{n}^{k}$ to be a stopping time which equals to the $k_{th}$ time period in the $n^{th}$ round. Then
\begin{align}
&\sum_{t=1}^{\infty}\mathbbm{1}\{c(t)>0\} \nonumber \\
=&\sum_{t=1}^{\infty}\sum_{n=1}^{\infty}\sum_{t=k}^{\infty}\mathbbm{1}\{c(t)>0\}\mathbbm{1}\{t=\tau_{n}^{k}\} \nonumber 
\end{align}

Thus, we know
\begin{align}
&E[\sum_{t=1}^{\infty}\mathbbm{1}\{c(t)>0\}] \nonumber \\
\leq &\sum_{n=n_{1}}^{\infty}\sum_{t=1}^{\infty}\sum_{k=1}^{\infty}P\left(\exists i,j:|u_{i}^{j}-u_{i,\tau_{n}^{k}}^{j}|>\frac{p^{-1}(\frac{p}{2})}{2Wm}\right)P(t=\tau_{n}^{k})+\sum_{n=1}^{n_1}N \nonumber \\
\leq &\sum_{n=n_{1}}^{\infty}\sum_{t=1}^{\infty}\sum_{k=1}^{\infty}24Nm \exp\left(\frac{-1.8 n\delta_{0}^2}{32m^2\sigma^2}\right)\times P(t=\tau_{n}^{k})+\sum_{n=1}^{n_1}N \nonumber \\
