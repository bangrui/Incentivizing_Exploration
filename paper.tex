...
\begin{lemma}
For all $n\geq 1$ and $l\geq 2$, we have
\begin{align}
...
\Longleftarrow &  1.3 n^{2/3} \geq  \log(\log_{1.1}(n)+1) \nonumber
\end{align}
\end{proof}
...
\leq & \sum_{n=1}^{n_{0}}NRn + \sum_{n=n_{0}}^{T}E\left[\sum_{t=t_{n}}^{t_{n+1}-1}24Nm\exp\left(-\frac{1.8n f(t)^2}{32 m^2\sigma^2}\right) R\right]+ \sum_{k=1}^{T}Mf(t)^2 \nonumber \\
