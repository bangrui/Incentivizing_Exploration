...
\begin{lemma}
Define $\tau$ to be any random variable that is almost surely between $t_n$ and $t_{n+1}-1$, then for all the agents whose utility difference between their best and second best arm is at least $2m\lambda$, we have $P(A_{\tau}\neq B(\theta_{\tau}))\leq 24Nm\exp\left(-\frac{1.8n\lambda^2}{8\sigma^2}\right)$, for $n\geq n_{0}=\max\{50, \frac{23.04\sigma^4}{\lambda^4}\}$.
\label{round:prob}
\end{lemma}
...
In the $n^{th}$ round, we know all arms have been pulled at least $n$ times. Thus, for all the agents whose utility difference between their best and second best arm is greater than $2m\lambda$, we have
\begin{align}
&P(A_{\tau}\neq B(\theta_{\tau}))\nonumber \\
\leq &P(\exists i, \exists j, |u_{i,\tau}^{j}-u_{i}^{j}|>\lambda) \nonumber \\
\leq & \sum_{i}\sum_{j} P(|u_{i,\tau}^{j}-u_{i}^{j}|>\lambda) \nonumber \\
= &  \sum_{i}\sum_{j} P(|\epsilon_{i,\tau}^{j}|>N(i,t)\lambda). \label{ACI}
\end{align}
...
\begin{align}
&P(|\epsilon_{i,\tau}^{j}|>N(i,t)\lambda) \nonumber \\
=&2P\left(S_{N(i,\tau)}^{i,j}>\frac{N(i,\tau)\lambda}{\sqrt{2}\sigma}\right) \nonumber \\
\leq & 2P\left(S_{N(i,\tau)}^{i,j}> \sqrt{0.6 N_{i,\tau}\log(\log_{1.1}(N(i,\tau))+1)+b N(i,\tau)}\right) \nonumber \\
\leq & 24\exp( -1.8b) \nonumber \\
...
Thus, we know
\begin{align}
&\sum_{i}\sum_{j} P(|\epsilon_{i,\tau}^{j}|>N(i,t)\lambda)  \nonumber \\
