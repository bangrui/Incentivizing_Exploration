...
\label{sec:prob}
Suppose we have $N$ arms. For the $i^{th}$ arm, it has a latent utility vector $u_i\in \mathbb{R}^{m}$. As the principal, we hope to learn each arm's latent utility vector $u_i$ through the interaction with the agents. We denote $u_{i,t}$ as our estimation for $u_i$ at time t. 
...
Throughout this paper, we abbreviate ``agent with preference vector $\theta$'' as just ``$\theta$''. We denote the agent comes to the system at time $t$ as $\theta_{t}$. For simplicity, we assume each type of agents has a unique favourite arm, denoted agent $\theta$'s best arm index as $B(\theta)$. 



At each time $t$, an agent comes to the system and can observe our current estimation $u_{i,t}$ for all arms. As the principal, we know $F(\cdot)$ from the prior knowledge. However, we do not know $\theta_t$.
...
Throughout this paper, we assume $m\geq 2$. For $m=1$,  \cite{han2015incentivizing} provides a complete characterization for its solution.
...
Throughout this section, we assume $F(\cdot)$ is a continuous distribution. Denote

\begin{align}
\Omega_{i,j}=\{\theta:B(\theta)=i, \hat{B}(\theta)=j\}, \nonumber 
\end{align}
which is the users whose best arm is arm $i$ and the second best arm is arm $j$. We need the following assumption in our analysis:

\begin{assumption} Let $f_{i,j}(y)$ be the density of $(u_i-u_j)\cdot\theta$ conditioned on $\theta \in \Omega_{i,j}$. We assume $M^{'}=\sup_{i,j}\limsup_{y\rightarrow 0^{+}}f_{i,j}(y) <\infty$.
\label{A1}
\end{assumption}

Intuitively, assumption~\ref{A1} states that there are not many agents who are indifferent between their best arm and the second best arm. For simplicity in our analysis, we use a slightly stronger condition below: $M=\sup_{i,j}sup_{y\in R^{+}}f_{i,j}(y) <\infty$.

\begin{assumption} We assume $F$ has a compact support set. Without loss of generality, we assume $\theta\in [0,W]^m$.
\label{A2}
\end{assumption}

Throughout this write up, we use $R$ to denote the maximum regret it can occur at each time.



\subsection{Unknown $p$ Value}
