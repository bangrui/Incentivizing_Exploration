...
\begin{lemma}
Define $\tau$ to be any stopping time that is almost surely between $t_n$ and $t_{n+1}-1$ with respect to the filteration $\mathcal{F}_{t}=\sigma(A_1,\cdots,A_t,c_1,\cdots,c_t,O_1,\cdots,O_t)$, we have 
\begin{align}
P(\arg\max\{\theta_{\tau}\cdot u_{i,\tau}\}\neq B(\theta_{\tau})|\theta_{\tau}\cdot(u_{B(\theta_{\tau})}-u_{\hat{B}(\theta_{\tau})})\geq 2Wm\lambda)\leq 24Nm\exp\left(-\frac{1.8n\lambda^2}{8\sigma^2}\right), \nonumber
\end{align}
for $n\geq n_{0}=\max\{50, \frac{23.04\sigma^4}{\lambda^4}\}$.
\label{round:prob}
\end{lemma}

Denote $n(\tau)$ to be the round number of $\tau$, then we can restate the above lemma as
\begin{align}
P(\arg\max\{\theta_{\tau}\cdot u_{i,\tau}\}\neq B(\theta_{\tau})|\theta_{\tau}\cdot(u_{B(\theta_{\tau})}-u_{\hat{B}(\theta_{\tau})})\geq 2Wm\lambda)\leq 24Nm\exp\left(-\frac{1.8n(\tau)\lambda^2}{8\sigma^2}\right), \nonumber
\end{align}
for $n(\tau)\geq n_{0}$. For simplicity, we use $n=n(\tau)$.
...
Let $X_{i}$ be zero mean $1/2$-subgaussian random variables. $\{S_{n}=\sum_{i=1}^{n}X_{i},n\geq 1\}$ be a random walk. Let $J$ be any stopping time with respect to $\{X_1,X_2,\cdots\}$. We let $J$ take a special value $\infty$ where $P(J=\infty)=1-\lim_{n\rightarrow \infty}P(J\leq n)$. If
\begin{align}
...
\geq & -Wm\lambda - Wm\lambda + 2Wm\lambda = 0,\nonumber
\end{align}
which means their myopic action would incur no regret.
\pfcomment{If all of these are equality, can the agent choose a regret-incurring option?  Do we need strict inequalities?}


Thus, we have
\begin{align}
&P(\arg\max\{\theta_{\tau}\cdot u_{i,\tau}\}\neq B(\theta_{\tau})|\theta_{\tau}\cdot(u_{B(\theta_{\tau})}-u_{\hat{B}(\theta_{\tau})})\geq 2Wm\lambda)\nonumber \\
\leq &P(\exists i, \exists j, |u_{i,\tau}^{j}-u_{i}^{j}|>\lambda |\theta_{\tau}\cdot(u_{B(\theta_{\tau})}-u_{\hat{B}(\theta_{\tau})})\geq 2Wm\lambda \nonumber \\
\leq & \sum_{i}\sum_{j} P(|u_{i,\tau}^{j}-u_{i}^{j}|>\lambda|\theta_{\tau}\cdot(u_{B(\theta_{\tau})}-u_{\hat{B}(\theta_{\tau})})\geq 2Wm\lambda) \nonumber \\
= &  \sum_{i}\sum_{j} P(|\epsilon_{i,\tau}^{j}|>N(i,t)\lambda|\theta_{\tau}\cdot(u_{B(\theta_{\tau})}-u_{\hat{B}(\theta_{\tau})})\geq 2Wm\lambda). \label{ACI}
\end{align}
...
\begin{align}
&P(|\epsilon_{i,\tau}^{j}|>N(i,t)\lambda|\theta_{\tau}\cdot(u_{B(\theta_{\tau})}-u_{\hat{B}(\theta_{\tau})})\geq 2Wm\lambda) \nonumber \\
=&2P\left(S_{N(i,\tau)}^{i,j}>\frac{N(i,\tau)\lambda}{\sqrt{2}\sigma}\right|\theta_{\tau}\cdot(u_{B(\theta_{\tau})}-u_{\hat{B}(\theta_{\tau})})\geq 2Wm\lambda) \nonumber \\
\leq & 2P\left(S_{N(i,\tau)}^{i,j}> \sqrt{0.6 N_{i,\tau}\log(\log_{1.1}(N(i,\tau))+1)+b N(i,\tau)}\right|\theta_{\tau}\cdot(u_{B(\theta_{\tau})}-u_{\hat{B}(\theta_{\tau})})\geq 2Wm\lambda) \nonumber \\
\leq & 24\exp( -1.8b) \nonumber \\
...
Thus, we know
\begin{align}
&\sum_{i}\sum_{j} P(|\epsilon_{i,\tau}^{j}|>N(i,t)\lambda|\theta_{\tau}\cdot(u_{B(\theta_{\tau})}-u_{\hat{B}(\theta_{\tau})})\geq 2Wm\lambda)  \nonumber \\
...
Fix $i$.
% and let $t^{0}=t_{n-1}+\sum_{j=1}^{i-1}X_{j}$. $X_{i}$ is a random variable that counts the number of trials before the $i^{th}$ unique arm is pulled, with trials occuring at times $t=t^{0},t^{0}+1,\cdots$. 
In bounding $X_i$, we think of agents as ``trials'', where each trial can result in a new unique arm being pulled (which we call a ``successful'' trial), or not.  There are two ways a trial can be successful:
\begin{itemize}
...
\begin{proof}
Denote $\epsilon_{i,t}=u_{i,t}-u_{i}$ to be the estimation error for the attribute vector $u_i$ at time $t$. Denote $\epsilon_{i,t}^{j}$ to be the $j^{th}$ component of $\epsilon_{i,t}$. Denote $\omega$ to be a sample path and $n(t,\omega)$ to be the round number for sample path $\omega$ at time $t$. Define
\begin{align}
W^{'}[l] = \{\omega:|\epsilon_{i,t}^{j}(\omega)|\leq g(n(t,\omega),l), \forall i,j\}\nonumber
\end{align}
...
Based on the definition of $W[l]$, we know if $\omega_t\in W[l]$, the maximum payment we need to offer at time $t$ is bounded above by $R+2Wmg(n,l)$.  \pfcomment{Do we need the $W$ here?  Can we drop it?} Thus,
