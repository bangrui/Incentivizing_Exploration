...
Let $A$ be the set of types satisfying 
$\AgV \cdot (\ArmV{\Best{\AgV}} - \ArmV{\Second{\AgV}}) \leq \EarlyCut{s}$.
By Assumption~\ref{A1}, and because $\EarlyCut{s} \leq \TieCutoff$,
the total measure of $A$
is $\AlmostTied{\EarlyCut{s}} \leq \TieDensity \cdot \EarlyCut{s}$,
Then, by Lemma~\ref{lem:phase-length},
the expected number of pulls in phase $s$ by agents in $A$  
bounded above by $\TieDensity\ \EarlyCut{s} \ARMNUM \log(s)$, and the total expected number of pulls by these agents by $\sum_{s=1}^T \TieDensity \ \EarlyCut[2]{s} \ARMNUM \log(s)$.
Consider any agent $\AgV$ incurring positive regret less than $\EarlyCut{s}$.
This agent's type must be in $A$.
Moreover, this agent's regret is bounded above by $\EarlyCut{s}$.
Thus, the expected total regret from such agents is at most
\begin{equation}
\sum_{s=1}^{T} \TieDensity \cdot \EarlyCut[2]{s} \ARMNUM \log(s).
...
under \AccEU{\tau_s^k}{\frac{\EarlyCut{s}}{2\Diam d}}, no agent at time $t$ will incur
regret more than \EarlyCut{s}.
...
\pfedit{Summing over the $T$ time periods,}
\pfdelete{Now, applying Lemma~\ref{lem:phase-length},}
the total expected regret from large-regret steps is at most
$\pfedit{24 \ARMNUM d \MAXR / T.}$
%\pfcomment{I deleted the left-hand side of the above equation because it was using random terms in the summand, and actually wasn't true.  What was intended was an expectation outside of the sum, but I thought it simpler just to delete that term.}
