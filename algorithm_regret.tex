...
\dkedit{Because $\frac{s}{\log s} = \Omega(s^{2/3})$, there are at most
...
\dkedit{$O(\ARMNUM \MAXR \cdot \Diam^3 d^3 \sigma^3/\TieCutoff^3
\cdot \log (\Diam^3 d^3 \sigma^3/\TieCutoff^3))
\leq O(\ARMNUM \MAXR \Diam^4 d^4 \sigma^4/\TieCutoff^4)
= O(\ARMNUM \MAXR)$,}
by crudely bounding \dkedit{$\log y \leq O(y^{1/3})$}.
...
\dkdelete{Because $n \leq T$, it is sufficient to show that
$\frac{50}{9} \log T \geq \log(\log_{1.1}(T) + 1)$,}
This can be verified by numerical calculation for \dkedit{$n=2$} and a
derivative test.
...
\dkedit{$24 \ARMNUM d \MAXR \cdot \frac{\pi^2}{6} = O(\ARMNUM \MAXR)$.}
