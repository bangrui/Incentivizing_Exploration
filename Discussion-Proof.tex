...
\item regret from phases with $\gamma(s) >= \hat{z}$ is $NR \log^2(T) D^4 d^4 \sigma^4 / \hat{z}^4$
\item regret from pulls with small regret is 0
\item regret from pulls with big regret is $NRd/T$
\end{itemize}

Total is NR ($\frac2p + \frac{d}{T} + \frac{\log^2(T) D^4 d^4 \sigma^4}{\hat{z}^4}$)
}
