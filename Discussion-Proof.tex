...
\begin{proof}
	The proof of the expected payment bound follows Lemma~\ref{rst:budget} \pfedit{ except we define 
	$h(\ell)$ without including the $\exp(2/\MinProb)$ term}
...
\pfedit{
The resulting bound on expected payment is
$O\left(\ARMNUM^2 d\cdot(R+Dd\sigma)\cdot \max\bigg\{ \left(\frac{\sigma \Diam d}{\TieCutoff} \right)^{5/2},
 ...
 = O\left(N^2 \max\{1, (L/p)^{5/2}\}\right)$.
}
...
O\left(\frac{\ARMNUM^2 \TieDensity^3 \Diam^3 d^3 \sigma^3}{\MinProb^2}
    + \frac{\ARMNUM^2 d^3 \sigma^2 \Diam^2}{\hat{z}^2} + \frac{\ARMNUM \Diam^3 d^3 \sigma^3}{\hat{z}^3}
 \right). \nonumber 
\end{align}
}
