...
\begin{itemize}
\item regret from times when we pay is $RN\frac2p$
\item regret from times with $\gamma(s) >= \hat{z}$ is $NR \log^2(T) D^4 d^4 sigma^4 / \hat{z}^4$
\item regret from small regret is 0
\item regret from big regret is $NRd/T$
\end{itemize}

Total is NR ($\frac2p + \frac{d}{T} + \frac{\log^2(T) D^4 d^4 sigma^4}{zhat^4}$)
}
...
Under Assumption~\ref{a:known-p}, the Known-\MinProb Algorithm has an expected payment budget bounded above by 
    \bcedit{
\begin{align*}
    O\left(\ARMNUM^2 d\cdot(R+Dd\sigma)\cdot \max\bigg\{ \left(\frac{24\sigma \ell \Diam d}{\TieCutoff} \right)^{5/2},
        \left( \frac{48 \sigma \ell \TieDensity \Diam d}{\MinProb} \right)^{5/2}\bigg\}\right)
\end{align*}
}
