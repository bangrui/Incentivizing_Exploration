...
\pfedit{(Our assumption of a continuous noise distribution and one free pull ensure that 
ties in $\AgV \cdot \ArmEV{t}{i}$ between arms occur with probability $0$.)} 
\end{itemize}
...
Because \pfedit{each arm has enough agents that prefer its true attribute vector}, 
...
Similarly, \pfedit{instantaneous} regret will decrease, and mostly accrue
due to ``problematic'' agents who are nearly tied in their preferences
between their top two \pfedit{arms}.
The detailed analysis following this outline is \pfedit{complicated by}
...
(and hence, by \pfedit{a} union bound, \AccEU{t}{x})
