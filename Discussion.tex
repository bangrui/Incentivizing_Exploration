...
Under Assumption~\ref{a:known-p}, the Known-\MinProb Algorithm has expected payment \pfdelete{budget} bounded above by 
    \bcedit{$O(\ARMNUM^2\max\{1,(\TieDensity/\MinProb)^{5/2}\}$ }
and expected regret bounded above by
\bcedit{
\begin{align*}
    O\left(\frac{\ARMNUM^2}{\MinProb^2} + \frac{\ARMNUM^2}{\hat{z}^2}+\frac{\ARMNUM}{\hat{z}^3}+\frac{\ARMNUM \TieDensity \log^3(T)}{\MinProb}\right).
\end{align*}
}
\end{theorem}
