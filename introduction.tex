...
\pfedit{substantial exploration of options could provide higher future rewards.}
However, individual users interact with the site a limited number
of times, and therefore have little incentive to explore.
\pfedit{Recent work has focused on the setting where users interact only once, 
and so have no intrinsic incentive to explore.}

Two recent lines of work have shown that
effecting a societally \pfedit{near-optimal} outcome in this setting \pfedit{requires
...
\pfedit{advantage to induce exploration by recommending apparently sub-optimal arms,
...
\pfedit{(as on a review-sharing site)}.
They suppose the principal can \pfedit{incentivize exploration} by offering \pfedit{arm-specific reward payments}.
\pfedit{We follow this second model.}

Past work has assumed \pfedit{users have homogeneous preferences over arms, i.e., the expected
reward derived from an arm is the same for all users}.
(\citet{han2015incentivizing} \pfedit{models users that} are
heterogeneous in their tradeoff between utility derived from arm pulls
and utility derived from the principal's payment,
\pfedit{but assumes preferences across arms are homogenous.})
...
videos, travel experiences), and not items \pfedit{with} a universally
agreed-on best order.
This is perhaps because regimes with heterogeneous preferences are the
ones where \pfedit{people have the most difficulty discovering} the best items, 
...
\pfedit{In this work, we present the first algorithm and analysis (of which we
are aware) for incentivizing exploration when users have heterogeneous 
preferences over arms.}
\pfedit{Our analysis provides insight into the
...
\pfedit{making her unaware of agents' preferred arms.
...
On the other hand, heterogeneity also \pfedit{offers}
the possibility of ``free exploration.''
Even \pfdelete{when left} unincentivized, agents will
...
\pfedit{and where} effecting essentially any exploration at all requires incentives.
...
offering \pfedit{arm-specific payments}.
