...
\pfedit{(Although linear, this model permits additional flexibility through the addition of derived attributes that are nonlinear transformation of some original attribute vector: define $\Agent{t}{i} = f_i(\AgentV{t})$ for some known nonlinear functions $f_i$ and $i=d+1,\ldots,d'$, and then increase $d$ to $d'$.)}
...
\dkedit{For a justification of the assumption that agents can observe
...
constant factor.
\dkdelete{(our algorithm depends on this quantity).}
...
\dkedit{Notice that based on Assumption~\ref{A1} below,
...
\dkedit{$\TieDensity, \TieCutoff \geq \frac{\MinProb}{2 \TieDensity}$
such that 
...
\dkedit{In particular, we consider $d$ a constant, because users
