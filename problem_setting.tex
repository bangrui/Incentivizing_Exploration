...
\dkcomment{Are we actually stating the result in this generality now?
  ...
  then we should just define everything for Gaussians here.}}
...
\dkcomment{It is unique with probablity 1 over the draw of agent types
  because for z=0, Assumption~\ref{A1} says that there is a measure 0
  set of agents whose difference between the best and second-best
  option is z=0. Or am I misunderstanding something?}
...
that is, there exist constants \dkedit{$\TieCutoff > 0, \TieDensity$} such that 
...
the key parameters, \dkedit{and also give some prominence to
