...
determined as $\ArmV{i} + \NoiseV$, where \NoiseV is a mean-zero independent \pfedit{continuous} sub-Gaussian%
\footnote{For simplicity of notation, we assume that the variance \pfedit{proxy}
  $\sigma^2$ is uniform across time steps.
  \pfedit{The} analysis extends \pfedit{easily when the variance proxy changes over time}.}
