...
\pfedit{(Although linear, this model permits additional flexibility through the addition of derived attributes that are nonlinear transformation of some original attribute vector: define $\Agent{t}{i} = \Psi_i(\AgentV{t})$ for some known nonlinear functions $\Psi_i$ where $i=d+1,\ldots,d'$, and then increase $d$ to $d'$.)}
\pfcomment{I added some detail.  Is it clear now David?  I'm just talking about a standard trick in linear models, where you add nonlinear transformations of your feature vector and have a linear model in higher dimensions.}
...
\pfcomment{Why is \Second{\AgV} unique with probability 1?  I don't think that is implied.  Can I remove that part of the statement?}
...
$\TieDensity, \TieCutoff \geq \frac{\MinProb}{2 \TieDensity}$
such that 
$\AlmostTied{z} \leq \TieDensity \cdot z$ for all $z \leq \TieCutoff$.
\end{assumption}

The requirement that $\TieCutoff \geq \frac{\MinProb}{2 \TieDensity}$
is not restrictive.
Because \TieDensity and \TieCutoff are both constants,
we could simply increase \TieDensity.
Perhaps even more simply, $\MinProb \leq 1/\ARMNUM$,
while \TieCutoff is a constant,
so an alternative is to decrease \MinProb by a suitable constant factor.
...
\pfedit{and each type has a unique best arm.}
...
the key parameters, while we consider
$\sigma, \Diam, d, \TieDensity, \TieCutoff$ to be constant.
...
\pfcomment{We report Thm 1 with $L$ in it.  Maybe we can still leave this sentence like it is.  I'll revisit later, if someone else doesn't resolve this first.}
In particular, we consider $d$ a constant, because users
typically evaluate products (or restaurants) by a small number of
relevant features.
