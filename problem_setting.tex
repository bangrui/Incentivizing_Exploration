...
\pfedit{
We assume above that the distribution $\AgentDist$ over preference vectors is known.  In practice, one would estimate this from agents' selections and the attribute estimates and offered payments on which they were based.  While we do not show it, we hypothesize that our regret guarantees only change by constants as long as our estimate of the conditional probability of pulling an arm is correct within a constant factor (our algorithm depends on this quantity).
}
...
Let $\MinProb = \min_{i} \Prob[\AgentDist]{\Best{\AgV} = i}$
...
We emphasize that this assumption implies ties happen with probability $0$.
