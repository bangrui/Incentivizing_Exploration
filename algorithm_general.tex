...
\dkcomment{If we only get 12 pages in the proceedings, make sure to
  replace ``appendix'' by ``online appendix'' or ``supplemental
  material'' or whatever we call it, here and everywhere else.}
...
i.e., satisfying $\dkedit{\tau_s} \in [t_s, t_{s+1})$ almost surely.
\dkcomment{$\tau$ was missing the subscript $s$ here. I think that was
  a mistake, but am marking it so someone can check.}
